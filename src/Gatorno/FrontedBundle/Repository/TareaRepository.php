...
class TareaRepository  extends EntityRepository{
    ...
    public function findCantPagosHoy($fechaInicio, $fechaFin)
    {
        ...
        $consulta = $em->createQuery('
                        SELECT p FROM FrontedBundle:Pago p
                        WHERE p.fechaDePago >= :fechaInicio
                        AND p.fechaDePago <= :fechaFin');
	  ...
	  public function findPorFecha($fechaInicial,$fechaFinal)
    {
        ...
        $consulta = $em->createQuery('SELECT t FROM FrontedBundle:Tarea t WHERE
        t.fechaDeCumplimiento >= :fechaInicial AND
        t.fechaDeCumplimiento <= :fechaFinal ORDER BY t.fechaDeCumplimiento ASC');
